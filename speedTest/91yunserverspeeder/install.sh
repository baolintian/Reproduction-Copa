...
# read shortRttMS
wget -N --no-check-certificate -O ip.log myip.ipip.net > /dev/null 2>&1
IP=$(cat ip.log | awk -F ' ' '{print $2}' | awk -F '：' '{print $2}')
if [ "$IP" == "" ]; then
	wget -N --no-check-certificate -O ip.log ip.cn > /dev/null 2>&1
	IP=$(cat ip.log | awk -F ' ' '{print $2}' | awk -F '：' '{print $2}')
fi
